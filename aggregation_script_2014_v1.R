...
fileList2014 <- list.files(path="D:\\praca_dyplomowa\\dane\\2014", pattern="*.csv", full.names=T, recursive=FALSE)
...
for(i in 1:length(fileList2014)){
  
  table1 <- read.csv(fileList2014[i], sep=",", header=F, skip = 2)[ ,c(2:3,5,13,18)]
  ...
  table1 <- table1[(as.character(table1$pickup_time)!=as.character(table1$dropoff_time)),]
...
ts_results2014 <- ts_results[-(1),]

rm(list=setdiff(ls(), "ts_results2014"))
write.table(ts_results2014, file = "D:\\praca_dyplomowa\\dane\\2014\\ts_results2014.csv", 
